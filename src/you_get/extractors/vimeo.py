...
def vimeo_download_by_id(id, title=None, output_dir='.', merge=True, info_only=False, **kwargs):
    ...
    try:
        ...
        video_page = get_content(config_url, headers=headers)
        ...
        video_page = get_content('http://player.vimeo.com/video/%s' % id, headers=headers)
    ...
    if not info_only:
        download_urls([url], title, ext, size, output_dir, merge=merge, headers=headers)
